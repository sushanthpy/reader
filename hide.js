...
function toggle(spanid) {
  if (getspan(spanid).innerHTML == "") {
    ...
    getspan(spanid + "indicator").innerHTML = '<img src="<?php echo $CFG->wwwroot; ?>/mod/reader/open.gif" alt="Opened folder" />';
  } else {
    vh_content[spanid] = getspan(spanid).innerHTML;
    getspan(spanid).innerHTML = "";
    getspan(spanid + "indicator").innerHTML = '<img src="<?php echo $CFG->wwwroot; ?>/mod/reader/closed.gif" alt="Closed folder" />';
  }
}
