...
class reader {
     ...
     */
    public static function create($readerid, $userid, $book) {
        global $DB;
        ...
        $book = $DB->get_record('reader_publisher', array('id' => $book), '*', MUST_EXIST);
    ...
    public function attempt_url($attemptid, $page = 0) {
        global $CFG;
        $url = $CFG->wwwroot . '/mod/reader/attempt.php?attempt=' . $attemptid;
        if ($page) {
            $url .= '&page=' . $page;
        }
        return $url;
    ...
    public function start_attempt_url($page = 0) {
        ...
        return new moodle_url('/mod/reader/startattempt.php', $params);
    }
...
class reader_attempt {
     ...
     * @param array $conditions passed to $DB->get_record('reader_attempts', $conditions).
     */
    public static function create_helper($conditions) {
        global $DB;

        $attempt = $DB->get_record('reader_attempts', $conditions, '*', MUST_EXIST);
        $readerpublisher = $DB->get_record('reader_publisher', array('id' => $attempt->quizid), '*', MUST_EXIST);
        $quiz = $DB->get_record('quiz', array('id' => $readerpublisher->quizid), '*', MUST_EXIST);
    ...
    public function summary_url() {
        return new moodle_url('/mod/reader/summary.php', array('attempt' => $this->attempt->id));
    }
    ...
    public function processattempt_url() {
        return new moodle_url('/mod/reader/processattempt.php');
    }
    ...
    public function finish_attempt($timestamp) {
        ...
        $this->attempt->sumgrades    = $this->quba->get_total_mark();
        $this->attempt->persent      = round(((float)$this->quba->get_total_mark() / (float)$this->readerobj->quiz->sumgrades) * 100);
    ...
    public function page_and_question_url($script, $slot, $page, $showall, $thispage) {
        ...
        if ($thispage == $page) {
            ...
            $url = new moodle_url('/mod/reader/' . $script . '.php' . $fragment,
...
function reader_questions_in_reader($layout) {
    $questions = str_replace(',0', '', reader_clean_layout($layout, true));
    if ($questions === '0') {
        return '';
    } else {
        return $questions;
...
/*
function reader_get_user_attempts($readerid, $userid, $status = 'finished') {
    global $DB;
    ...
    return $DB->get_records_select('quiz_attempts',
            'reader = ? AND userid = ?' . $status_condition[$status],
            array($readerid, $userid), 'attempt ASC');
}


function reader_create_attempt($reader, $attemptnumber, $lastattempt, $timenow, $quiz) {
    global $USER;
    ...
    if ($attemptnumber == 1) {
        ...
        $attempt->preview = 0;
        $attempt->layout = reader_clean_layout($quiz->questions, true);
        if ($quiz->shufflequestions) {
            $attempt->layout = reader_repaginate($attempt->layout, $quiz->questionsperpage);
        }
    }
